...
nori deploys other services by watching a GHCR image, and on a new digest running a
bash script that pulls the image and swaps the container(s). It cannot do this to itself:
a normal deploy script runs `docker stop nori && docker rm nori && docker run …`
**from inside the nori container**, so `docker stop nori` kills the very process
running the script, mid-line. The new container never starts. This can't be dodged with
`nohup`/`setsid`/`disown`, because `docker stop nori` tears down the whole container's
PID namespace — any detached child inside it dies too.

The goal: let nori update itself using the **same trigger and history mechanism** it
...
The swap must be performed by a process running **outside** the nori container. We
introduce a one-shot **launcher**: the nori image invoked with `up` / `update`
subcommands. The launcher **owns nori's run config** and is what creates the nori
container in the first place. Because the launcher created nori, the "update" path
...
exits. There is no long-running supervisor and no IPC channel — nori is alive at the
...
1. **Failure safety: minimal.** No health check, no auto-rollback. A bad image leaves
   nori down; recovery is manual.
2. **Launcher is one-shot**, not a resident supervisor.
3. **Launcher is the nori image + subcommands** (`up`, `update`), not a second image —
   one artifact, launcher improves in lockstep with the app. The image already ships `bash`
   + the `docker` CLI.
4. **Config lives in a bespoke file on a shared volume** (`nori-config` at `/config`),
...
### 1. Launcher subcommands (`cmd/nori`)

`main.go` already dispatches subcommands off `os.Args[1]` (`hash-password`, `seed-demo`).
Add:

- **`up`** — bootstrap (first boot) then create/recreate the nori container from config.
- **`update`** — swap the running nori container to a target digest.
...
`nori-config` volume, mounted at `/config`. Single source of truth, human-editable,
holds two concerns:
...
- **Run spec:** image ref (tag, e.g. `ghcr.io/tomusdrw/nori:latest`), container
  ...
  (incl. `nori.service=<self-service-name>`), restart policy.
- **App env:** `NORI_KEY`, `NORI_SESSION_KEY`, `NORI_ADMIN_HASH`, plus any
...
1. Generate `NORI_KEY` and `NORI_SESSION_KEY` (32 random bytes, base64).
2. **Prompt for the admin password** over the TTY, bcrypt-hash it via the existing
   `auth.HashPassword` → `NORI_ADMIN_HASH`. Non-interactive escape hatch
   (`--admin-password-hash <hash>` flag or env var) for scripted installs.
3. Write the config (run spec + env) to `/config`.
4. Create the nori container from the spec, injecting the self-identity env
   (`NORI_CONFIG_VOLUME`, `NORI_SELF_CONTAINER`, `NORI_SELF_IMAGE`) and the
   `--label nori.service=<self>` label.

**Idempotent:** if config already exists, skip steps 1–3 entirely and just (re)create from
it. This rule is what guarantees `NORI_KEY` is generated exactly once — regenerating it
...
propagate it to nori as `NORI_CONFIG_VOLUME`; nori needs the name (not a
mount) to spawn the updater with the volume attached.

### 4. Self-service seeding (nori boot)

When nori starts and detects the launcher-managed env vars, it **idempotently ensures a
self-service row** exists:

- name: reserved (e.g. `nori`), watched image = `NORI_SELF_IMAGE`,
...
self-identity vars `$NORI_SELF_IMAGE` and `$NORI_CONFIG_VOLUME` are **not** in the
script environment by default: `OSRunner.Run` sets `cmd.Env` without inheriting nori's
process env (`executor.go:42`). So the executor, when building the **self-service's** script
env, must read these two values from nori's own process env (set by the launcher at
...
(exit ≠ 0), the normal `failed` path applies, since nori is still alive to record it.

### 6. `update` — inside the detached updater container

1. Read config from `/config`.
2. `docker pull <image>@<target-digest>`.
3. `docker stop <name>` + `docker rm <name>` (the old nori; safe — separate container).
4. `docker run` the new nori from the config's run spec, **pinned to the target digest**,
...
1. Reads its own now-running digest (via the `nori.service=<self>` label, the same way
...
poller / scheduler / manual button
  ...
  → detached updater: pull <image>@<D>, stop+rm old nori, run new nori
  → old nori process dies during stop; new nori boots on <D>
...
- A broken new image → nori is down. Recover by hand:
  `docker run --rm -v …/docker.sock -v nori-config:/config <image>@<previous-digest> update --target-digest <previous-digest>`,
...
- **`cmd/nori/main.go`** — dispatch `up` / `update` (/ `rollback`) alongside the
...
generate fresh `NORI_KEY`/`NORI_SESSION_KEY` unless the operator seeds the config
